...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button partButton = (Button) findViewById(R.id.partCal);
        partButton.setOnClickListener(new View.OnClickListener(){

            @Override
            public void onClick(View v) {
                Intent intent = new Intent(MainActivity.this, PartCalculation.class);
                startActivityForResult(intent, 1000);
                Log.d("here","111");

                Intent getIntent = getIntent();

                String ans = getIntent.getStringExtra(PartCalculation.partAns);
                Log.d("here","asdf"+ans);
                main.setText(ans);
            }
        });
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(resultCode == Activity.RESULT_OK && requestCode == 1000){
            String message = data.getStringExtra(PartCalculation.partAns);
            main.setText(message);
            below = message;
            if(message.length()>0){
                touch=message.length();
            }
        }
    }
